// ============================================================
// 【学生自己编写的代码】路由注册文件（最重要的文件！）
// 作用：配置所有API的URL路径与处理函数的对应关系
// ============================================================
...
func mainFunc(ctx context.Context, parser *gcmd.Parser) (err error) {
	...
	middlewareSvc := middleware.New()   // 【自己写的】中间件

	// 第3步：注册全局中间件（所有请求都会经过）
	s.Use(ghttp.MiddlewareHandlerResponse)   // GoFrame内置：统一响应格式
	s.Use(middlewareSvc.Ctx)                 // 【自己写的】Ctx中间件：每个请求都解析用户身份
	s.Use(ghttp.MiddlewareCORS)              // CORS：允许跨域（前端3000端口调后端8000端口）

	// 第4步：创建控制器实例
	userCtrl := user.NewV1()     // 【自己写的】用户控制器
	bookCtrl := book.NewV1()     // 【自己写的】图书控制器
	borrowCtrl := borrow.NewV1() // 【自己写的】借阅控制器
	...
	s.Group("/api", func(group *ghttp.RouterGroup) {
		...
		group.Group("/", func(group *ghttp.RouterGroup) {
			group.Middleware(middlewareSvc.Auth)    // 【自己写的】检查登录状态

			// ------ 图书管理（需管理员权限）------
			group.Group("/", func(group *ghttp.RouterGroup) {
				group.Middleware(middlewareSvc.Admin)  // 【自己写的】检查管理员权限
