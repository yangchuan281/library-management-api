...
7. UpdatePhone（修改手机号）：更新当前登录用户的手机号，写入数据库
...
- email（邮箱，唯一）
- phone（手机号，唯一）
- phone_updated_at（手机号最后修改时间，用于限制修改频率）
- password（MD5加密后的密码）
...
- updatePhoneApi(phone)：调用PUT /api/users/me/phone
...
### Q10：怎么修改手机号？
答：在个人中心页面输入新手机号，点击保存。后端先检查新手机号是否已被其他用户注册，如果没有就更新当前用户的手机号。这个接口需要登录后才能调用，因为要确认是用户本人在修改自己的信息。
...
3. internal/service/ - 业务逻辑是我写的（图书CRUD、借书还书、用户注册登录、修改手机号）
